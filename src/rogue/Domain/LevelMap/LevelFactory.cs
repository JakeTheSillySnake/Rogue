  ...
  public class LevelFactory {
    ...
    private List<(int, int)> GenerateEnemySpawns(int[,] map, Random random, List<Room> rooms,
      ...
      for (int i = 0; i < difficulty; i++) {
        ...
        if (map[enemyPosY, enemyPosX] != (int)MapCellStates.EMPTY) {
          i--;
        } else {
    ...
    private List<(int, int)> GenerateItemSpawns(int[,] map, Random random, List<Room> rooms,
      ...
      for (int i = 10; i > difficulty; i--) {
        ...
        if (map[itemPosY, itemPosX] != (int)MapCellStates.EMPTY) {
          i--;
        } else {
    ...
    private void PrepareEnterAndExit(int[,] map, Random random, List<Room> rooms) {
      ...
      while (map[enterExitPosY, enterExitPosX] != (int)MapCellStates.EMPTY) {
    ...
    public bool IsDoorNearby(int[,] map, int posY, int posX) {
      ...
      for (int y = -1; y < 2; y++) {
        for (int x = -1; x < 2; x++) {
          isDoorNearby = isDoorNearby || (map[y + posY, x + posX] == (int)MapCellStates.DOOR &&
                                          Math.Abs(y + x) % 2 == 1);
        }
      }
      return isDoorNearby;
